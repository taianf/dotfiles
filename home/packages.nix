...
in
{
  ...
  # helmDefaults.args). helm-diff doesn't recognize that flag —
  # it's a `helm upgrade` flag, not a helm-diff one. Set the
  # standard helm-diff passthrough env var so unknown flags
  # flow through to helm instead of erroring. See
  # https://github.com/databus23/helm-diff/issues/278.
